...
def main():
    ...
    if mode == "active" and report.get("setups"):
        try:
            ...
            # ترفیع پولبک اول به سیگنال زنده (دستور صریح حمید، ۲۰ اوت —
            # «هیچ‌چیز در صف نماند»). پشتوانه: چهار روز پیاپی و سه بلوک‌بندی
            # مستقل، دفتر اول بهتر از دوم — انباشتهٔ بدون‌تکرار +0.270R
            # CI [+0.152,+0.387] در برابر دومِ +0.023R؛ تفاضل CI بلوکی-روزانه
            # [+0.058,+0.449] بالای صفر.
            #
            # قانون ۷ و ۸ رعایت است: استراتژیِ **جدا و نسخه‌دار** (hamid-p1)،
            # نه تغییرِ بی‌صدای قانون هسته — قانون «منتظر پولبک دوم» برای
            # استراتژی hamid سر جایش است و این کانال کارنامهٔ خودش را
            # می‌سازد؛ اگر رکورد زنده‌اش CI منفی داد، بسته می‌شود.
            # سقف ۳ در چرخه تا سهمیهٔ ۱۶تایی روز را از مسیر اصلی نگیرد؛
            # از همهٔ دروازه‌های گلوگاه (روند، قانون پایدار، هم‌زمانی،
            # بازجویی) مثل بقیه می‌گذرد — ترفیع، معافیت نیست.
            p1 = sorted([x for x in report["setups"] if x.get("waiting")],
                        key=lambda x: -(x.get("learn_score") or 0))[:3]
            for x in p1:
                one = _for_telegram(x)
                one["strategy"] = "hamid-p1"
                one["strategyName"] = ("روش حمید — ورود پولبک اول "
                                       "(کانال آزمایشی زنده، v1)")
                one["footer"] = ((one.get("footer") or "")
                                 + "\n<i>🧪 پولبک اول — پشتوانه: ۴ روز پیاپی "
                                   "CI بالای صفر روی دفتر بازپخش؛ کارنامهٔ "
                                   "زندهٔ خودش را می‌سازد.</i>")
                outs.append(one)
            if p1:
                act(f"🧪 پولبک اول: {len(p1)} ستاپ به گلوگاه ارسال رفت "
                    f"(استراتژی جدا hamid-p1)")
            for s_ in outs:
