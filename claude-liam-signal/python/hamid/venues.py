...
# ── دروازهٔ سخت: فقط نمادی که واقعاً در بیت‌یونیکس قابل معامله است ─────────
# دستور حمید (۱۵ اوت): «ارزهایی باشد که در بیت‌یونیکس موجود باشد.»
# و سند شخصی‌سازی: «سیگنال اجرایی فقط برای نمادی که در صرافی هدف — در
# وضعیت فعلی Bitunix — واقعاً قابل معامله و دارای نقدشوندگی کافی است.»
#
# این با `prioritize` فرق دارد و فرقش عمدی است: آن ترتیب را عوض می‌کرد،
# این عضویت را. تحلیل و تحقیق روی کل جهان ادامه دارد؛ فقط **ارسال** محدود
# می‌شود. سیگنالی که حمید نتواند بگیرد، سیگنال نیست.

EXECUTION_VENUE = "bitunix"


def tradable(sym, idx=None):
    """(مجاز؟, دلیل) — برای گلوگاه ارسال.

    وقتی فهرست صرافی نیامده باشد، پاسخ «نه» است نه «شاید». قانون ۱ سند
    غیرقابل‌مذاکره: دادهٔ ناقص/UNKNOWN در فیلد اجباری = NO_SIGNAL. قابل
    معامله بودن روی صرافیِ هدف یک فیلد اجباری است — اگر ندانیم، نمی‌فرستیم.
    """
    idx = idx if idx is not None else index()
    if not idx:
        return False, "فهرست بیت‌یونیکس در دسترس نیست — UNKNOWN یعنی NO_SIGNAL"
    vs = idx.get(sym) or []
    if EXECUTION_VENUE in vs:
        return True, ""
    if vs:
        return False, f"در بیت‌یونیکس نیست (فقط {'/'.join(vs)}) — ارسال نمی‌شود"
    return False, "روی هیچ صرافی هدف نیست — ارسال نمی‌شود"
