...
def c_panel_loads():
    ...
    # ۱۶ اوت: «aura liam mAx» هم اضافه شد. این چک باید هر دو خط تولید را
    # بشناسد — نه فقط آنی که موقع نوشتنش وجود داشت. تنها چیزی که این را از
    # آژیر کاذب نجات می‌داد «cycle v» بود، و تکیه به یک نشانهٔ اتفاقی برای
    # چکی که «پنل خراب است» فریاد می‌زند، همان چیزی است که یک بار قبلاً
    # آژیر کاذب داد.
    if all(m not in html for m in ("لیام تریدر ۹", "aura liam mAx",
                                   "حمید کلود مکس پنل", "cycle v")):
