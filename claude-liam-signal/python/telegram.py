...
# نشود.
# دستور حمید (۱۷ اوت): «تنها پنلی هستی که در گیت‌هاب auraliam18 دارم» —
# پنل دیگر به مخزن جدای خودش رفت. پس پیش‌فرضِ این مخزن، برند همین پنل
# است؛ متغیر محیطی فقط برای روز مبادا می‌ماند، نه برای جدا نگه داشتن دو
# پنل در یک مخزن (آن مسئله با جدایی مخزن‌ها حل شد).
PANEL_NAME = os.getenv("PANEL_NAME") or "aura liam mAx"
