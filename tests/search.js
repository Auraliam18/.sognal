...
const GRID={
  fresh:[15,10,20,25],          // bars since the structure break
  rmin:[1.8,1.5,2.2,2.6],       // target must pay at least this
  decisive:[0.5,0.35,0.7],      // break must clear this much ATR
  disp:[1.5,1.2,1.9]            // displacement leg, in ATR
};
function patch(c){
  return{
    'ob.brk<=(typeof smcFreshBars==="function"?smcFreshBars():15)':'ob.brk<='+c.fresh,
    'const RMIN=1.8;':'const RMIN='+c.rmin+';',
    'atr*0.5)':'atr*'+c.decisive+')',
    'atr*1.5)':'atr*'+c.disp+')'
  };
}
...
if(!isMainThread){
  ...
  console.log(`${list.length} configurations · fit on markets 1-${FIT}, judged on ${FIT+1}-${FIT+TEST} · ${CORES} cores`);
  ...
  Promise.all(jobs.map(w=>new Promise((res,rej)=>{
    ...
    const base=all.find(r=>r.c.fresh===15&&r.c.rmin===1.8&&r.c.decisive===0.5&&r.c.disp===1.5);
