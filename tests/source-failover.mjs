...
const REPLY = {
  ...
  "api.bybit.com":    ()=>JSON.stringify({result:{list:mk("bybit")}}),
...
function mk(kind){
  const out=[]; let t=1785500000000, p=63000;
  for(let i=0;i<60;i++){
    const o=p, c=p*(1+(i%7-3)*0.0004), h=Math.max(o,c)*1.0006, l=Math.min(o,c)*0.9994;
    if(kind==="bitunix") out.push({time:t,open:""+o,high:""+h,low:""+l,close:""+c,quoteVol:"10",baseVol:"630000"});
    else if(kind==="kucoin") out.push([""+(t/1000),""+o,""+c,""+h,""+l,"10","630000"]);
    else if(kind==="bybit"||kind==="okx") out.push([""+t,""+o,""+h,""+l,""+c,"10","630000"]);
    else if(kind==="bitget") out.push([""+t,""+o,""+h,""+l,""+c,"10","630000"]);
    else if(kind==="gate") out.push([""+(t/1000),"630000",""+c,""+h,""+l,""+o,"10"]);
    else out.push([t,""+o,""+h,""+l,""+c,"10",t+899999]);
    p=c; t+=900000;
  }
  if(kind==="kucoin"||kind==="bybit"||kind==="okx") out.reverse();   // newest first
  return out;
}
