...
## امضای پنل روی هر پیام (دستور حمید، ۱۶ اوت — به‌روزِ ۱۷ اوت)

هر پیام تلگرام — سیگنال، رسید، آلارم — امضای پنل فرستنده را دارد تا حمید
بداند از کدام پنل آمده. این‌جا: `telegram.PANEL_NAME` («aura liam mAx»؛
از ۱۷ اوت این مخزن تک‌پنل است — بند «هویت مخزن» بالا). رسید TradingView
برند را از فیلد `panel` خود آلارم می‌گیرد؛ آلارم بی‌امضا صریح «بدون
امضای پنل» می‌خورد — امضا حدس زده نمی‌شود. در JSON آلارم‌های TradingView
این پنل `"panel":"aura liam mAx"` بگذار (مقادیر قدیمی پذیرفته می‌مانند).
